...
namespace shop.Core.Factory
{
    public class FavoriteFactory
    {
        ...
        {
            string sql = @"INSERT INTO shop_favorite (uid,productid,createtime,productxml) VALUES (?uid,?productid,?createtime,?productxml)";
        ...
        {
            string sql = "delete from shop_favorite where fid=" + fid;
        ...
        public static List<FavoriteModel> GetListByUid(int uid, int pageSize, int pageIndex, out int totalcount)
        {
            ...
            string sql = string.Format("select * from shop_favorite where uid={0} order by fid desc limit {1},{2}", uid.ToString(), start, pageSize);
            ...
            totalcount = MySqlDbHelper.ExecuteScalar("select count(*) from shop_favorite where uid=" + uid);
        ...
        {
            int favcount = MySqlDbHelper.ExecuteScalar( string.Format("select count(*) from shop_favorite where uid={0} and productid={1}", uid, productid));
