...
namespace shop.Core.Factory
{
    public class OrderFactory
    {
        ...
        {
            string sql = @"INSERT INTO shop_order (orderno,uid,customername,tel,address,orderstatus,deliverstatus,createtime,orderxml) VALUES (?orderno,?uid,?customername,?tel,?address,?orderstatus,?deliverstatus,?createtime,?orderxml)";
        ...
        {
            string sql = "UPDATE shop_order set customername=?customername,tel=?tel,address=?address,orderstatus=?orderstatus,deliverstatus=?deliverstatus,createtime=?createtime,orderxml=?orderxml where orderid=?orderid";
            MySqlParameter[] parameters = { 
                MySqlDbHelper.MakeInParam("?customername" , MySqlDbType.VarChar , 50 ,info.customername) ,
                MySqlDbHelper.MakeInParam("?tel" , MySqlDbType.VarChar , 50 ,info.tel) ,
                MySqlDbHelper.MakeInParam("?address" , MySqlDbType.VarChar , 100 ,info.address) ,
        ...
        {
            string sql = "UPDATE shop_order set orderstatus=?orderstatus where orderid=?orderid";
            MySqlParameter[] parameters = {
        ...
        {
            string sql = "UPDATE shop_order set deliverstatus=?deliverstatus where orderno=@orderno";
            MySqlParameter[] parameters = { 
        ...
        {
            string sql = "UPDATE shop_order set orderstatus=?orderstatus where orderno=?orderno";
            MySqlParameter[] parameters = { 
                MySqlDbHelper.MakeInParam("?orderstatus" , MySqlDbType.Int32 , 4 ,orderstatus),
                MySqlDbHelper.MakeInParam("?orderno" , MySqlDbType.VarChar , 32 ,orderno)
            };
        ...
        {
            string sql = "select * from shop_order where orderid=" + orderid;
        ...
        {
            string sql = "select top 5 * from shop_order where orderstatus=5 and uid=" + uid + " order by orderid desc";
        ...
        {
            string sql = string.Format("select * from shop_order where orderno='{0}'", orderno);
        ...
        {
            string sql = "select * from shop_order where orderstatus=1 and uid=" + uid;
        ...
        {
            string sql = "select * from shop_order where orderstatus=5 and uid=" + uid + " order by orderid desc limit 5"; ;
        ...
        public static DataTable GetList(int pageSize, int pageIndex, string whereSql, string orderBy, out int totalcount)
        {
            ...
            string sql = string.Format("select * from shop_order {0}{1} limit {2},{3}", whereSql, orderBy, start, pageSize);

            DataTable dt = MySqlDbHelper.Query(sql).Tables[0];
            totalcount = MySqlDbHelper.ExecuteScalar(string.Format("select count(*) from shop_order {0}", whereSql));
        ...
        {
            string sql = string.Format("select * from shop_order {0} order by orderid desc limit {1}", whereSql,count);
        ...
        {
            string sql = string.Format("select * from shop_order {0} order by orderid desc", whereSql);
