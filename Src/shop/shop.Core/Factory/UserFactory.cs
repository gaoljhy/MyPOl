...
namespace shop.Core.Factory
{
    ...
    public class UserFactory
    {
        ...
        {
            string sql = @"INSERT INTO shop_user(nickname,openid,fullname,sex,tel,address,headpicurl,jbnum,jfnum,createdate,fromuid,usertype) VALUES(?nickname,?openid,?fullname,?sex,?tel,?address,?headpicurl,?jbnum,?jfnum,?createdate,?fromuid,?usertype)";
        ...
        {
            string sql = "delete from shop_user where uid=" + uid.ToString();
        ...
        {
            string sql = @"update shop_user set usertype=?usertype where uid = ?uid";
            MySqlParameter[] parameters = { 
        ...
        public static void UpdateJb(int uid, int jb)
        {
            ...
            {
                jbnum = jb;
                sql = @"update shop_user set jbnum=jbnum+?jbnum where uid = ?uid";
            }
            else
            {
                jbnum = -jb;
        ...
        public static void UpdateJf(int uid, int jf)
        {
            ...
            {
                jfnum = jf;
                sql = @"update shop_user set jfnum=jfnum+?jfnum where uid = ?uid";
            }
            else
            {
                jfnum = -jf;
        ...
        {
            string sql = "select * from shop_user where uid=" + uid;
        ...
        {
            string sql = "select * from shop_user order by uid asc limit 1";
            DataTable dt = MySqlDbHelper.Query(sql).Tables[0];
        ...
        {
            string sql = "select * from shop_user where openid='" + openid + "'";
            DataTable dt = MySqlDbHelper.Query(sql).Tables[0];
        ...
        public static DataTable GetList(int pageSize, int pageIndex, string whereSql, string orderBy, out int totalcount)
        {
            ...
            string sql = string.Format("select * from shop_user {0}{1} limit {2},{3}", whereSql, orderBy, start, pageSize);

            DataTable dt = MySqlDbHelper.Query(sql).Tables[0];
            totalcount = MySqlDbHelper.ExecuteScalar(string.Format("select count(*) from shop_user {0}", whereSql));
        ...
        {
            string sql = string.Format("select * from shop_user {0} order by uid desc", whereSql);
        ...
        {
            int totalcount = MySqlDbHelper.ExecuteScalar("select count(*) from shop_user ");
            return totalcount;
        }
