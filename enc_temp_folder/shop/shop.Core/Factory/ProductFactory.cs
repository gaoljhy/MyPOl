...
namespace shop.Core.Factory
{
    public class ProductFactory
    {
        ...
        {
            string sql = "delete from shop_product where productid=" + productid;
            MySqlDbHelper.ExecuteSql(sql);
        }

        public static void UpdateHits(int productid)
        {
            string sql = "UPDATE shop_product set hits=hits+1 where productid=" + productid;
            MySqlDbHelper.ExecuteSql(sql);
        }
        ...
        {
            string sql = "select * from shop_product where productid=" + productid;
        ...
        public static List<ProductModel> GetList(int pageSize, int pageIndex, string whereSql, string orderBy, out int totalcount)
        {
            ...
            string sql = string.Format("select * from shop_product {0}{1} limit {2},{3}", whereSql, orderBy, start, pageSize);

            DataTable dt = MySqlDbHelper.Query(sql).Tables[0];
            totalcount = MySqlDbHelper.ExecuteScalar(string.Format("select count(*) from shop_product {0}", whereSql));
        ...
        {
            string sql = string.Format("select * from shop_product {0} order by productid desc limit {1}", whereSql,count);
        ...
        {
            string sql = string.Format("select * from shop_product {0} order by productid desc", whereSql);
